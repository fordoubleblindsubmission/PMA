...
typename traits::value_type CPMA<traits>::value(key_type e) const {
  ...
  // TODO deal with the value for zero
  if (e == 0) {
    if (has_0) {
      assert(false);
      // TODO deal with the value of 0
      return {};
    } else {
      return {};
    }
...
// TODO merge serial and parallel versions
template <typename traits>
[[nodiscard]] uint64_t
...
CPMA<traits>::get_leaf_bounds(uint64_t split_points) const {
  ...
  // TODO see why this can't be made parallel
...
uint64_t CPMA<traits>::insert_batch(element_ptr_type e, uint64_t batch_size) {
  ...
  // TODO make it work for the first batch
  if (get_element_count() == 0) {
    uint64_t count = 0;
  ...
  // TODO currently only works for unsigned types
  while (e.get() == 0) {
    has_0 = true;
    ++e;
    batch_size -= 1;
